...
class MockServer {
    ...
    // MARK: - Mocks
    func respondToGetCurrencyRatesWithSuccess(mockFileName: String = "getCurrencyRates") {
        URLCache.shared.removeAllCachedResponses()
        webServer.addHandler(forMethod: "GET", path: "/api/live", request: GCDWebServerRequest.self, processBlock: { request in
            var data: Data!
            do {
    ...
    func respondToGetCurrencyRatesWithError(statusCode: Int = 500) {
        URLCache.shared.removeAllCachedResponses()
        webServer.addHandler(forMethod: "GET", path: "/api/live", request: GCDWebServerRequest.self, processBlock: { request in
